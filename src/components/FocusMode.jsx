...
export default function FocusMode({ cards, onClose, onCardDone }) {
  const pom = usePomodoro()

  // Load today's focus selection from localStorage
  const todayKey = `focus-${new Date().toISOString().slice(0,10)}`
  const [selectedIds, setSelectedIds] = useState(() => {
    try {
      const saved = JSON.parse(localStorage.getItem(todayKey))
      // filter to only cards that still exist
      if (Array.isArray(saved)) return saved.filter(id => cards.find(c => c.id === id))
    } catch {}
    return []
  })
  const [picking,      setPicking]    = useState(selectedIds.length === 0)
  // Bug 1 fix: seed from actual card stage so re-opening preserves done state
  ...
  const focusCards = selectedIds.map(id => cards.find(c => c.id === id)).filter(Boolean)
      ...
      <div style={s.body}>
        ...
        {allDone ? (
          ...
          <>
            ...
            {/* Card manager — always visible so cards can be swapped at any time */}
