...
import { X, Plus, Trash2, Link, ExternalLink, Loader } from 'lucide-react'
...
export default function CardModal({ card, onSave, onClose }) {
  ...
  const [stage,    setStage]    = useState(card?.stage  || 'todo')
  ...
  const addLink = async () => {
    ...
    let pageTitle = url
    try {
      // Use a CORS-friendly title fetch via allorigins
      const res = await fetch(`https://api.allorigins.win/get?url=${encodeURIComponent(url)}`)
      const data = await res.json()
      const match = data.contents?.match(/<title[^>]*>([^<]+)<\/title>/i)
      if (match) pageTitle = match[1].trim()
    } catch { /* silently fall back to URL */ }
    setFetching(false)
  ...
  return (
    <div style={s.overlay} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={s.modal} className="modal-enter">
        ...
        {/* Header */}
        <div style={s.header}>
          <span style={s.heading}>{card?.id ? 'Edit card' : 'New card'}</span>
...
const s = {
  ...
  subAddRow: { display: 'flex', gap: 6, marginTop: 2 },
